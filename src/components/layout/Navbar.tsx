...
export function Navbar({ trips = [] }: { trips?: Trip[] }) {
  ...
  const navLinks = [
    { label: "Leh Ladakh", href: "/trips", items: lehTrips },
    { label: "Spiti Valley", href: "/trips", items: spitiTrips },
    { label: "Other Trips", href: "/trips", items: otherTrips },
            ...
            <nav className="hidden lg:flex items-center gap-8 xl:gap-10">
              {navLinks.map((link) => (
                <div key={link.href + link.label} className="relative group">
                  <Link
                    href={link.href}
                    className={`font-nav text-[11px] font-medium uppercase tracking-[0.18em] transition-colors flex items-center gap-1 py-4 ${
            ...
            <nav className="flex flex-col items-center gap-6">
              ...
              >
                ...
                <Link
                  href="/trips"
                  onClick={() => setMobileOpen(false)}
                  className="font-nav text-xs font-medium uppercase tracking-widest px-8 py-3 bg-accent text-white hover:bg-accent-dark transition-all"
                >
                  Reserve
                </Link>
              </motion.div>
